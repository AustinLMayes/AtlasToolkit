...
public class XMLInventoryUtils {
  ...
  public static HashMap<Integer, ItemStack> inventoryToHashMap(Player p) {
    ...
    for (int slot = 0; slot < 36; slot++) {
      for (ItemStack i : p.getInventory().getContents()) {
        if (p.getInventory().getItem(slot) == null || i == null) {
          continue;
        }
  ...
  public static String itemToXML(ItemStack stack, Integer slot, String tagName) {
    ...
    final StringBuilder builder = new StringBuilder("<" + tagName + " ")
    ...
    if (meta != null && meta.hasDisplayName()) {
      builder.append(
          XMLUtils.generateAttribute("name", meta.getDisplayName().replace("§", "`"), true));
    }

    if (stack.getType().toString().startsWith("LEATHER_")) {
      LeatherArmorMeta leatherMeta = (LeatherArmorMeta) stack.getItemMeta();
      String hex = String.format("%02x%02x%02x", leatherMeta.getColor().getRed(),
          leatherMeta.getColor().getGreen(), leatherMeta.getColor().getBlue());
      if (hex.length() < 6) {
    ...
    builder.append(XMLUtils
        .generateAttribute("material", stack.getType().name().toLowerCase().replace('_', ' '),
            false));
    ...
    if (meta instanceof PotionMeta && ((PotionMeta) meta).hasCustomEffects()) {
      ...
      ((PotionMeta) meta).getCustomEffects().forEach(e ->
          builder
              .append("<effect " + XMLUtils.generateAttribute("amplifier", e.getAmplifier(), true) +
    ...
    if (meta != null && meta.hasEnchants()) {
      ...
      meta.getEnchants().entrySet().forEach(l -> builder.append("<enchantment level=\"" +
          l.getValue() + "\">" + l.getKey().getName().toLowerCase().replace("_", " ")
          + "</enchantment>\n"));
